...
package com.zlm.audio.formats.mp3;
 ...
 * @Date: 12.06.2009
 */
public class MP3Decoder implements com.zlm.audio.Decoder {
	...
	private int samplesToMinutes(long samples) {
		return (int) (samples / audioInfo.getSampleRate() / 60f);
	}
	...
	public boolean open(final AudioInfo audioInfo) {
		if (audioInfo == null)
			return false;
		this.audioInfo = audioInfo;
		try {
			// URI location = audioInfo.getLocation();
			InputStream fis;
			// if (audioInfo.isFile()) {
			// logger.fine("Opening file: " + trackData.getFile());
			streaming = false;
			fis = new FileInputStream(audioInfo.getFile());
			streamSize = audioInfo.getFile().length();
			// } else {
			// trackData.setCodec("MP3 Stream");
			// logger.fine("Opening stream: " +
			// URLDecoder.decode(location.toString(), "utf8"));
			// streaming = true;
			...
			oldBitrate = audioInfo.getBitrate();
			samplesPerFrame = (int) (header.ms_per_frame() * header.frequency() / 1000);
			// audioFormat = new AudioFormat(sampleRate, 16, channels, true,
			// false);
	...
	public int decode(byte[] buf) {
		try {
			...
			if (readFrame.bitrate_instant() > 0)
				audioInfo.setBitrate(readFrame.bitrate_instant() / 1000);
			...
			currentSample += AudioMathUtil.bytesToSamples(len,
					audioInfo.getFrameSize());

			if (!streaming && currentSample > totalSamples) {
				len -= AudioMathUtil.samplesToBytes(currentSample
						- totalSamples, audioInfo.getFrameSize());
			}
	...
	public void close() {
		if (bitstream != null)
			bitstream.close();
		audioInfo.setBitrate(oldBitrate);
		readFrame = null;
	}
