 ...
 * @Date: 26.06.2009
 */
public class MP3FileReader extends com.zlm.audio.AudioFileReader {
	...
	public AudioInfo readSingle(AudioInfo audioInfo) {
		...
		try {
			mp3File = new MP3File(audioInfo.getFile(), MP3File.LOAD_ALL, true);
		...
		if (mp3File != null) {
			...
			copyHeaderFields(mp3AudioHeader, audioInfo);

			long totalSamples = audioInfo.getTotalSamples();
	...
	// private void setMusiqueTagFieldValues(Track track, FieldKey key,
	// ID3v24Tag tag) {
	...
	// track.getTrackData()
	// .addRating(String.valueOf(body.getRating()));
	...
	// track.getTrackData().addTagFieldValues(key,
	// body.getValueAtIndex(i));
	// }
	// }
	// }
	// }
